...
/*   Updated: 2023/06/05 20:21:08 by iamongeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_rdata_consts(t_cub *cub, t_rcast *rc, t_rdata *rd, t_pdata *pd)
{
	...
	while (++i < SCN_WIDTH)
	{
		...
		rd[i].pcx = &cub->hero.ply_obj->cx;
		rd[i].pcy = &cub->hero.ply_obj->cy;
		rd[i].px = &cub->hero.ply_obj->px;
		rd[i].py = &cub->hero.ply_obj->py;
...
int	init_raycaster(t_cub *cub)
{
	...
	rcast->pdata = malloc(sizeof(t_pdata) * SCN_WIDTH);
	if (!rcast->theta_offs || !rcast->ray_thetas || !rcast->ray_thetas
		|| !rcast->rays[0] || !rcast->rays[1] || !rcast->rdata || !rcast->pdata)
	...
	init_rdata_consts(cub, rcast, rcast->rdata, rcast->pdata);
	return (update_fov(cub, FOV));
}
